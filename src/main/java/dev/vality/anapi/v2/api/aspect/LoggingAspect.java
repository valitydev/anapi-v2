package dev.vality.anapi.v2.api.aspect;
...
public class LoggingAspect {
    ...
    @Pointcut("within(dev.vality.anapi.v2.api..*) && within(*..*DelegateService)")
    ...
    public void logMethod(JoinPoint joinPoint) {
        log.info("-> Req: {}", getParameters(joinPoint));
    }
    ...
    private String getParameters(JoinPoint joinPoint) {
        ...
        for (int i = 0; i < parameterNames.length; i++) {
            stringBuilder.append(parameterNames[i]).append(NAME_VALUE_DELIMITER).append(joinPoint.getArgs()[i])
